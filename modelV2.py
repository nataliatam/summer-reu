...
class WirelessCell:
    def __init__(self, x, y, type):
        self.x = x
        self.y = y
        self.type = type # types: SHARED, PATH, NONE, PURPLE
...
def drawCells():
    for i in range(len(WirelessList)):
        if (WirelessList[i].type == "SHARED" or WirelessList[i].type == "RED"):
            center = plt.Circle((WirelessList[i].x, WirelessList[i].y), 1,  color = 'r')
    ...
    for i in range(len(finalLeaves)):
        ...
        if (min[0] == finalLeaves[i][0]):
            if(min[1] >= finalLeaves[i][1]): 
                ...
                while newY < min[1]:
                    addWireless(finalLeaves[i][0], newY, "RED")
                    newY += wlR
            else:
                newY = finalLeaves[i][1] - wlR
                while newY > min[1]:
            ...
            if(min[0] >= finalLeaves[i][0]): 
                ...
                while newX < min[0]:
                    addWireless(newX, finalLeaves[i][1], "RED")
                    newX += wlR
            else:
                newX = finalLeaves[i][0] - wlR
                while newX > min[0]:
